...
export const handler = async(event) => {
  ...
  if (getLeagueInfoResponse.$metadata.httpStatusCode !== 200 || !getLeagueInfoResponse.Item) {
    return {
      getLeagueInfoResponse: getLeagueInfoResponse,
      statusCode: 500
    }
  }
  ...
  // First we will set the league status in the league table from active to playoffs
  ...
  if (findCurrentLeagueMembersResponse.$metadata.httpStatusCode !== 200) {
    return {
      findCurrentLeagueMembersResponse: findCurrentLeagueMembersResponse,
      statusCode: 500
    }
  }
  ...
  for (const leagueMember of findCurrentLeagueMembersResponse.Items) {
    ...
    if (numWins >= minimumWinsToQualify) {
      ...
      if (updatePastLeaguesResponse.$metadata.httpStatusCode !== 200) {
        return {
          updatePastLeaguesResponse: updatePastLeaguesResponse,
          statusCode: 500
        }
      }
    }
    ...
    if (updateCurrentLeagueResponse.$metadata.httpStatusCode !== 200) {
      return {
        updateCurrentLeagueResponse: updateCurrentLeagueResponse,
        statusCode: 500
      }
    }
  } 
